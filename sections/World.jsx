'use client';
import { motion } from "framer-motion";
import styles from '../styles';
import { staggerContainer, fadeIn } from '../utils/motion';
import { TypingText, TitleText } from '../components';

const World = () => (
  <section>
    <motion.div
    variants={staggerContainer}
    initial='hidden'
    whileInView='show'
    viewport={{once: false, amount: 0.25}}
    className={`${styles.innerWidth} mx-auto flex flex-col`}>
      <TypingText title="| People on the World" textStyles="text-center" />
      <TitleText title={<>Track friends around you and invite them to play together in the same world</>} textStyles='text-center' />
      <motion.div
        variants={fadeIn('up', 'tween', 0.3, 1)}
        className="relative mt-[68px] flex w-full h-[550px]">
        <img src="/map.png" alt="map" className="w-full h-full object-cover" />
        <div className='absolute bottom-20 md:right-[10%] right-20 w-[70px] h-[70px] p-[6px] rounded-full bg-[#5d6680]'>
          <img src="/people-01.png" alt="people 1" className='w-full h-full' />
        </div>
        <div className='absolute top-10 left-40 w-[70px] h-[70px] p-[6px] rounded-full bg-[#5d6680]'>
          <img src="/people-02.png" alt="people 2" className='w-full h-full' />
        </div>
        <div className='absolute top-1/2 left-[45%] w-[70px] h-[70px] p-[6px] rounded-full bg-[#5d6680]'>
          <img src="/people-03.png" alt="people 3" className='w-full h-full' />
        </div>
        <div className='absolute top-1/2 left-[17%] w-[15%] h-[15%]p-[6px] rounded-[20px]'>
          <img src="/card1.svg" alt="card 1"  />
          <h3 className='absolute bottom-0 p-6 pt-[25px] justify-start mt-[30px] font-semibold sm:text-[15px] text-[12px] text-white'>
            The Upside Down
          </h3>
        </div>
        <div className='absolute top-[15%] left-[60%]  w-[15%] h-[15%]p-[6px] rounded-[20px]'>
          <img src="/card2.svg" alt="card 2"  />
          <h3 className='absolute bottom-0 p-6 pt-[25px] justify-start mt-[30px] font-semibold sm:text-[15px] text-[12px] text-white'>
            HawkinsLabs
          </h3>
        </div>
      </motion.div>
    </motion.div>
  </section>
);

export default World;
